...
#计算possible bulk flux, qb, 其中5为flux scaling parameter(martin and kok sa), 0.277m/s为起动摩擦风速(Utfv=sqrt(g*d*rho_s/rho_f)/10, 其中g为重力加速度9.81m/s2, d=0.00036m(360微米)为中值粒径(martin and kok sa和中国沙样d50(432个沙丘230微米)的平均, 取mm单位小数点后两位)(matin and kok 2017和中国沙样d50(432个沙丘230微米)的平均, 取mm单位小数点后两位), rho_s=2650kg/m3为沙粒密度, rho_f=1.22kg/m3为空气密度，最后得到起动摩擦风速Utfv=0.277m/s. bagnold 1941).
...
cdo expr,'qb_E=qb*sin(qa);' ea5_land_merge_"$((y))".nc era5_land_qb_E_"$((y))".nc
